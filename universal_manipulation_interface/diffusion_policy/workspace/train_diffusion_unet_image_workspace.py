...
class TrainDiffusionUnetImageWorkspace(BaseWorkspace):
    ...
    def __init__(self, cfg: OmegaConf, output_dir=None):
        ...
        # Handle both single and dual encoder policies
        ...
        if hasattr(self.model, 'obs_encoder'):
            ...
            # Dual encoder case (image_obs_encoder + tactile_obs_encoder)
            if hasattr(cfg.policy, 'image_obs_encoder') and cfg.policy.image_obs_encoder.pretrained:
                obs_encorder_lr *= 0.1
                print('==> reduce pretrained image_obs_encorder\'s lr')
            if hasattr(cfg.policy, 'tactile_obs_encoder') and cfg.policy.tactile_obs_encoder.pretrained:
                obs_encorder_lr *= 0.1
                print('==> reduce pretrained tactile_obs_encorder\'s lr')
            
            # Add parameters from both encoders
            if hasattr(self.model, 'image_obs_encoder'):
                for param in self.model.image_obs_encoder.parameters():
                    if param.requires_grad:
                        obs_encorder_params.append(param)
            if hasattr(self.model, 'tactile_obs_encoder'):
                for param in self.model.tactile_obs_encoder.parameters():
                    if param.requires_grad:
                        obs_encorder_params.append(param)
            ...
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                if cfg.training.freeze_encoder:
                    ...
                    if hasattr(self.model, 'obs_encoder'):
                        ...
                        # Dual encoder case
                        if hasattr(self.model, 'image_obs_encoder'):
                            self.model.image_obs_encoder.eval()
                            self.model.image_obs_encoder.requires_grad_(False)
                        if hasattr(self.model, 'tactile_obs_encoder'):
                            self.model.tactile_obs_encoder.eval()
                            self.model.tactile_obs_encoder.requires_grad_(False)
